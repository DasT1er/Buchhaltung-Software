    ...
    <>
      ...
      <div className="p-4 sm:p-6 space-y-3">
        ...
        <div className="glass rounded-lg p-2.5 flex items-center justify-between">
          <span className="text-sm font-bold text-d-on-tint uppercase tracking-wide">Gesamt ({filtered.length})</span>
          <span className="text-lg font-black text-d-on-tint">{formatCurrency(gesamt)}</span>
        </div>
                ...
                {filtered.map(item => (
                  <tr key={item.id} className="hover:bg-card-alt/30 transition-colors">
                    ...
                    <td className="px-4 py-2 hidden md:table-cell">
                      <span className="inline-flex px-2 py-1 text-xs font-bold bg-w-tint/80 text-w-on-tint rounded-md backdrop-blur-sm">
                        {getAusgabeKategorieLabel(item.kategorie)}
                      </span>
                    </td>
                    <td className="px-4 py-2 text-xs text-muted hidden lg:table-cell">{getZahlungsartLabel(item.zahlungsart)}</td>
                    <td className="px-4 py-2 text-right text-sm font-bold text-danger-600 dark:text-danger-400 whitespace-nowrap">
