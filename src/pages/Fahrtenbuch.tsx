  ...
  return (
    <>
      <Header
        ...
        actions={
          <div className="flex gap-2">
            <button onClick={() => exportFahrtenCSV(filtered)} className="hidden sm:flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-body bg-card-alt rounded-lg hover:opacity-80 transition-colors">
      ...
      <div className="p-4 sm:p-6 space-y-4">
        ...
        <div className="bg-p-tint border border-p-tint-border rounded-xl p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Car size={20} className="text-p-on-tint" />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-divider bg-card-alt">
                  ...
                  <th className="text-left px-4 py-3 font-semibold text-body">Route</th>
                  <th className="text-left px-4 py-3 font-semibold text-body hidden md:table-cell">Zweck</th>
                  <th className="text-left px-4 py-3 font-semibold text-body hidden lg:table-cell">Kunde</th>
                  <th className="text-right px-4 py-3 font-semibold text-body">Kilometer</th>
              ...
              <tbody className="divide-y divide-divider-light">
                {filtered.map(item => (
                  <tr key={item.id} className="hover:bg-card-alt transition-colors">
                    ...
                    <td className="px-4 py-3 text-body hidden md:table-cell">{item.zweck}</td>
                    <td className="px-4 py-3 text-muted hidden lg:table-cell">{item.kunde || '-'}</td>
                    <td className="px-4 py-3 text-right font-semibold text-heading whitespace-nowrap">{item.kilometer.toLocaleString('de-DE')} km</td>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-12 text-center text-muted">Keine Fahrten gefunden. Klicke auf "Neue Fahrt" um zu beginnen.</td></tr>
                )}
              </tbody>
            </table>
          </div>
